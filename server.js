    ...
    try {
      ...
      if (activeSessions.length < AUTO_LOOP_CONFIG.maxSessions) {
        ...
        const sessionConfig = {
          ...
          isAutoLoop: true,
          maxRestarts: 5,
          proxyType: 'web' // Default ke web proxy untuk auto-loop
        };
...
app.post('/api/start-session', async (req, res) => {
  try {
    console.log('Starting new session with PROXY ENFORCEMENT:', {
      ...req.body,
    ...
    // VALIDASI: Proxy type harus disertakan
    if (!proxyType) {
      return res.status(400).json({
        success: false,
    ...
    const sessionConfig = {
      ...
      proxyType: proxyType || 'web' // Default ke web jika tidak ada
    };

    console.log('Session config with PROXY ENFORCEMENT');
    ...
    res.json({ 
      success: true, 
      sessionId,
      message: 'Session started with PROXY ENFORCEMENT'
    });
  } catch (error) {
