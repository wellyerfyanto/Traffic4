...
class TrafficGenerator {
  ...
  // SIMPLIFIED Browser Health Check - Fokus pada connection saja
  async checkBrowserHealth(browser, sessionId) {
    try {
      if (!browser) return false;
      
      // Simple check: coba ambil pages
      const pages = await browser.pages();
      return true; // Jika tidak error, anggap sehat
    } catch (error) {
  ...
  async setupAndTestProxiesForSession(sessionId, config) {
    ...
    try {
      ...
      this.log(sessionId, 'PROXY_SELECTED', 
        `Selected ${proxyType} proxy: ${this.getProxyDisplay(selectedProxy)} - Type: ${selectedProxy.type || selectedProxy.protocol} - Country: ${selectedProxy.country || 'unknown'}`
      );
      
      // Tambah log detail untuk fresh proxy
      if (proxyType === 'fresh' && selectedProxy.ip && selectedProxy.port) {
        this.log(sessionId, 'PROXY_DETAILS', 
          `Fresh proxy details: ${selectedProxy.ip}:${selectedProxy.port} | Protocol: ${selectedProxy.protocol || 'http'} | Source: ${selectedProxy.source || 'unknown'}`
        );
      }
  ...
  async rotateProxyForSession(sessionId, config) {
    try {
        ...
        if (newProxy) {
            ...
            this.log(sessionId, 'PROXY_ROTATED', 
              `Rotated to new proxy: ${this.getProxyDisplay(newProxy)} (${newProxy.type || newProxy.protocol})`);
        }
    } catch (error) {
  ...
  // SIMPLIFIED Browser Launch - tanpa health check yang strict
  async launchBrowser(sessionId, config) {
    try {
      ...
      const args = [
        ...
        '--disable-features=AdBlock', // PASTIKAN TANPA BLOKIR IKLAN
      ...
      } else if (config.proxyType === 'vpn') {
        this.log(sessionId, 'VPN_EXTENSION', 'Using VPN extension (simulated)');
      }
  ...
  // FITUR-FITUR PENUH TANPA BLOKIR IKLAN:
  async executeAllFeatures(page, sessionId, config) {
    const features = [
      ...
      {
        name: 'VIEW_ADS', // Ubah dari HANDLE_ADS menjadi VIEW_ADS (tanpa blokir)
        action: async () => {
      ...
      },
      {
        name: 'CLEANUP',
        action: async () => {
          this.log(sessionId, 'FEATURE_CLEANUP', 'Cleaning up cache and cookies...');
          await this.clearCacheAndCookies(page, sessionId);
        },
        timeout: 5000
      }
    ];
  ...
  async viewAdsForImpressions(page, sessionId) {
    try {
      ...
      await page.evaluate(() => {
        const adElements = document.querySelectorAll('[class*="ad"], [id*="ad"], .adsbygoogle, .ad-container, .advertisement, [data-ad]');
        if (adElements.length > 0) {
          console.log(`🔍 Found ${adElements.length} ad elements for impressions`);
          ...
          adElements.forEach((ad, index) => {
            if (index < 3) { // Hanya 3 pertama
              try {
                ...
                // Tambah class highlight untuk simulasi
                ad.style.outline = '2px solid #ff9900';
                setTimeout(() => {
                  ad.style.outline = '';
                }, 1000);
              } catch (e) {
  ...
  // FITUR 9: Cleanup (tanpa menghapus iklan)
  async clearCacheAndCookies(page, sessionId) {
    try {
      const client = await page.target().createCDPSession();
      await client.send('Network.clearBrowserCache');
      await client.send('Network.clearBrowserCookies');
      this.log(sessionId, 'CACHE_CLEARED', 'Cache and cookies cleared (ads remain for impressions)');
    } catch (error) {
