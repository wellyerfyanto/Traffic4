...
class TrafficGenerator {
  ...
  async executeAllFeatures(page, sessionId, config) {
    const features = [
      ...
      },
      {
        name: 'HANDLE_ADS',
        action: async () => {
          this.log(sessionId, 'FEATURE_ADS', 'Handling Google ads...');
          await this.handleGoogleAds(page, sessionId);
        },
        timeout: 30000
      },
      {
  ...
  // FITUR 4: Click Multiple Post Links
  ...
  // FITUR 5: Handle Google Ads
  async handleGoogleAds(page, sessionId) {
    try {
      // First, try to find and click ads (for ad revenue)
      ...
      if (adClicked) {
        this.log(sessionId, 'AD_CLICKED', 'Clicked on ad');
        await page.waitForTimeout(8000);
        await page.goBack();
        await page.waitForTimeout(5000);
      } else {
        // If no ads found, hide ad elements
        await page.evaluate(() => {
          const adElements = document.querySelectorAll('[class*="ad"], [id*="ad"], .ad-container, .adsbygoogle');
          adElements.forEach(el => {
            if (el.style) el.style.display = 'none';
          });
        });
        this.log(sessionId, 'ADS_HIDDEN', 'Ad elements hidden');
      }
    } catch (error) {
      this.log(sessionId, 'ADS_ERROR', `Ad handling error: ${error.message}`);
    }
  }
  ...
  async clearCacheAndCookies(page, sessionId) {
    try {
      ...
      this.log(sessionId, 'CACHE_CLEARED', 'Cache and cookies cleared');
    } catch (error) {
      // Silent fail
    }
  }
