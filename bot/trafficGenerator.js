...
class TrafficGenerator {
  ...
  async executeSession(sessionId, config) {
    let browser;
    try {
      this.log(sessionId, 'STEP_1', 'Launching browser for WEB PROXY...');
  ...
  async executeWebProxyFlow(page, sessionId, config) {
    ...
    try {
      this.log(sessionId, 'WEB_PROXY_START', `Starting web proxy: ${webProxy.name} (${webProxy.method})`);
      ...
      if (webProxy.method === 'direct_url' && webProxy.pattern) {
        ...
        await this.executeFormFillMethod(page, sessionId, config, webProxy);
      } else {
  ...
  async executeFormFillMethod(page, sessionId, config, webProxy) {
    this.log(sessionId, 'FORM_FILL_METHOD', `Using form fill method for ${webProxy.name}`);
    ...
    if (!inputFilled) {
      throw new Error('Cannot find URL input field');
    }

    const formSubmitted = await this.findAndSubmitForm(page, sessionId, webProxy);
    if (!formSubmitted) {
      await page.keyboard.press('Enter');
      this.log(sessionId, 'ENTER_FALLBACK', 'Used Enter key as fallback');
    }

    await page.waitForNavigation({ waitUntil: 'domcontentloaded', timeout: 15000 })
      .catch(() => this.log(sessionId, 'NAV_TIMEOUT', 'Navigation timeout, continuing...'));
  ...
  async findAndFillInput(page, sessionId, targetUrl, webProxy) {
    ...
    this.log(sessionId, 'INPUT_NOT_FOUND', 'No URL input field found');
    return false;
  }

  async findAndSubmitForm(page, sessionId, webProxy) {
  ...
  async tryUrlReconstruction(page, sessionId, config, webProxy) {
    ...
    for (const pattern of patterns) {
      try {
        ...
        // Continue ke pattern berikutnya
      }
    }
